...
--1 cho biet ten gv co kha nang day it mon hoc nhat
select MAGV
...
--2 cho biet magv sinh nam 1994 ma da tung duoc phan cong giang
-- day mon hoc ma minh khong co kha nang day
select gv.MaGV
from PHANCONG pc,  GIAOVIEN gv
where YEAR( gv.NgaySinh) ='1984'and pc.MaGV=gv.MaGV and pc.MaMH
not in (select gvdmh1.MaMH
					from GIAOVIEN gv1, GIAOVIEN_DAY_MONHOC gvdmh1
					where gv1.MaGV = gvdmh1.MaGV and gv1.MaGV = gv.MaGV)
             
--3 cho biet ma mon hoc da duoc mo ra cho tat ca cac lop bat dau nam 2010
select pc.MaMH 
from  PHANCONG pc,LOPHOC lh,GIAOVIEN gv ,GIAOVIEN_DAY_MONHOC gvdmh
where lh.MaLop=pc.MaLop and  gv.MaGV=gvdmh.MaGV and gv.MaGV=lh.GVQuanLi and lh.NamBatDau ='2010'and 
                             not exists (select MaLop
                                          from LOPHOC lh
                                          where lh.NamBatDau='2010' )
                                          except (select pc2.Malop 
                                          from PHANCONG pc2,PHANCONG pc
                                          where pc2.MaLop=pc.Malop)
  --4 
  create procedure sp_themkn @magv nchar (10),@mamh nchar (10),@thamnien  int
  as
  declare @solopdaday int
  set @solopdaday=(select SoLopDaDay 
                   from GIAOVIEN_DAY_MONHOC
                   where SoLopDaDay=@solopdaday )
  insert into GIAOVIEN_DAY_MONHOC values (@magv,@mamh,@thamnien,null)
  exec   
  create procedure sp_kiemtratontai @magv nchar(10)
  as
  if (exists (select * from GIAOVIEN where @magv=MaGV))
     print N'@magiaovientontai'
     else 
     print N'@magiaovienkhongtontai'+@magv
     exec sp_kiemtratontai 'GV00001'
     
     alter procedure sp_kiemtratontaiMaMH @mamh nchar(10)
  as
  if (exists (select * from MONHOC where @mamh=MaMonHoc))
     print N'@mamh tontai'
     else 
     print N'@mamh khongtontai'+@mamh
     exec sp_kiemtratontaiMaMH 'MH00001'
 --5 trigger 
 create trigger sllopmo
 on GIAOVIEN_DAY_MONHOC
 for update ,insert 
 as 
 if(update (MAMH))
 begin
 if exists (select * from inserted I, GIAOVIEN_DAY_MONHOC gvdmh where gvdmh.MaMH=I.MaGV and  exists 
 (select * from GIAOVIEN_DAY_MONHOC
 group by MaMH
 having COUNT (*)>2)
 begin raiserror ('N loi. ma mon hoc chi duoc toi da 2 lan',16,1)
 rollback 
 end
 end 
